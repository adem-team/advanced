...
$aryMachine = ArrayHelper::map(Machine::find()->all(),'TerminalID','MESIN_NM');
	 ...
	 */
	 $attDinamikRo=[];
	if($aryProviderDetailRo->allModels){
		//$cnt=count($aryProviderDetailRo->allModels)!=0?(count($aryProviderDetailRo->allModels)-1):0;
		foreach($aryProviderHeaderRo as $key =>$value){
