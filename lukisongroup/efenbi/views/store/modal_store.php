	...
	function tombolCreate(){
		if(getPermission()){
			if(getPermission()->BTN_CREATE==1){
				...
				$url = Url::toRoute(['/efenbi/store/create']);
				$options1 = ['value'=>$url,
	...
	function tombolView($url, $model){
		if(getPermission()){
			...
			if(getPermission()->BTN_VIEW==1 OR getPermission()->BTN_CREATE==1){
				...
				$options1 = [
					'value'=>url::to(['/efenbi/store/view','id'=>$model->ID]),
	...
	function tombolReview($url, $model){
		if(getPermission()){
			...
			if(getPermission()->BTN_REVIEW==1){
				...
				$options1 = [
					'value'=>url::to(['/efenbi/store/review','id'=>$model->ID]),
	...
	function tombolRemainder($url, $model){
		if(getPermission()){
			if(getPermission()->BTN_PROCESS1==1){
				...
				$url = url::to(['/efenbi/store/remainder','id'=>$model->ID]);
				$options1 = [
					'value'=>$url,
	...
	function tombolExportExcel(){
		// if(getPermission()){
			// if(getPermission()->BTN_PROCESS1==1){
				...
				$url = Url::toRoute(['/efenbi/store/export-excel']);
				$options1 = ['value'=>$url,
