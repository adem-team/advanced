...
echo $this->render('modal_itemFormula'); //echo difinition
	...
	//Header Attribute
	$bColorHeader='rgba(255, 179, 95, 0.9)';
	$gvAttributeItemHeader=[
		[
			'class'=>'kartik\grid\SerialColumn',
			'contentOptions'=>['class'=>'kartik-sheet-style'],
			'width'=>'10px',
			'header'=>'No.',
			'headerOptions'=>Yii::$app->gv->gvContainHeader('center','30px',$bColorHeader),
		...
		[
			...
			'noWrap'=>false,
			//gvContainHeader($align,$width,$bColor)
			'headerOptions'=>Yii::$app->gv->gvContainHeader('center','80px',$bColorHeader),
		...
		[
			...
			//gvContainHeader($align,$width,$bColor)
			'headerOptions'=>Yii::$app->gv->gvContainHeader('center','50',$bColorHeader),
		...
		[
			...
			'headerOptions'=>Yii::$app->gv->gvContainHeader('center','10px',$bColorHeader),
			'contentOptions'=>Yii::$app->gv->gvContainBody('center','10px',''),
		]
	];

	//Gridview Header
	$gvItemFormulaHeader=GridView::widget([
		'id'=>'gv-item-formula-header',
		'dataProvider' => $dataProvider,
		'rowOptions'   => function ($model, $key, $index, $grid) {
			return ['id'=>$model->ID,'onclick' => '$.pjax.reload({
				url: "'.Url::to([
							'/efenbi-rasasayang/item-formula/index'
						]).'?id='.$model->ID.'",
				container: "#gv-item-formula-detail",
				timeout: 1000,
			});'];
			//  return ['data-id' => $model->USER_ID];
		},
		'filterModel' => $searchModel,
		'columns'=>$gvAttributeItemHeader,				
		'pjax'=>true,
		'pjaxSettings'=>[
			'options'=>[
	...
	$bColorDetail='rgba(52, 235, 138, 1)';
	$gvAttributeItemDetail=[
		[
			'class'=>'kartik\grid\SerialColumn',
			'contentOptions'=>['class'=>'kartik-sheet-style'],
			'width'=>'10px',
			'header'=>'No.',
			'headerOptions'=>Yii::$app->gv->gvContainHeader('center','30px',$bColorDetail),
			'contentOptions'=>Yii::$app->gv->gvContainBody('center','30px',''),
		],
		//DISCOUNT_HARI.
		[
			'attribute'=>'DISCOUNT_HARI',
			//'label'=>'Cutomer',
			'filterType'=>true,
			'filterOptions'=>Yii::$app->gv->gvFilterContainHeader('0','80px'),
			'hAlign'=>'right',
			...
			'noWrap'=>false,
			//gvContainHeader($align,$width,$bColor)
			'headerOptions'=>Yii::$app->gv->gvContainHeader('center','80px',$bColorDetail),
			'contentOptions'=>Yii::$app->gv->gvContainBody('left','80px',''),
			
		],		
		//DISCOUNT_WAKTU1.
		[
			'attribute'=>'DISCOUNT_WAKTU1',
			//'label'=>'Cutomer',
			'filterType'=>true,
			...
			'noWrap'=>false,
			//gvContainHeader($align,$width,$bColor)
			'headerOptions'=>Yii::$app->gv->gvContainHeader('center','40px',$bColorDetail),
			'contentOptions'=>Yii::$app->gv->gvContainBody('left','40px',''),
			
		],		
		//DISCOUNT_WAKTU2.
		[
			'attribute'=>'DISCOUNT_WAKTU2',
			//'label'=>'Cutomer',
			'filterType'=>true,
			'filterOptions'=>Yii::$app->gv->gvFilterContainHeader('0','40px'),
			'hAlign'=>'right',
		...
		[
			...
			//gvContainHeader($align,$width,$bColor)
			'headerOptions'=>Yii::$app->gv->gvContainHeader('center','30',$bColorDetail),
			'contentOptions'=>Yii::$app->gv->gvContainBody('center','30','')			
		],
		//ACTION
		[
			'class' => 'kartik\grid\ActionColumn',
			...
			'headerOptions'=>Yii::$app->gv->gvContainHeader('center','10px',$bColorDetail),
			'contentOptions'=>Yii::$app->gv->gvContainBody('center','10px',''),
		]
	];
	
	//Gridview Detail
	$gvItemFormulaDetail=GridView::widget([
		'id'=>'gv-item-formula-detail',
		'dataProvider' => $dataProviderDetail,
		'filterModel' => $searchModelDetail,
		'columns'=>$gvAttributeItemDetail,				
		'pjax'=>true,
		'pjaxSettings'=>[
			'options'=>[
				'enablePushState'=>false,
...
<div class="container-fluid" style="font-family: verdana, arial, sans-serif ;font-size: 8pt">
	<div class="col-xs-6 col-sm-6 col-lg-6" style="font-family: tahoma ;font-size: 9pt;">
		<div class="row">
			<?=$gvItemFormulaHeader?>
		</div>
	</div>
