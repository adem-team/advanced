...
class SalesDetailController extends Controller
{
    ...
    public function actionIndex()
    {
		...
		//$plsql_so_1= new ActiveDataProvider([
			/*   'query' => $this->getScripts1(),
			'pagination' => [
					'pageSize' => 24,
			],  */
			'key' => 'ID',
		...
		$attributeField=$plsql_so_1->allModels[0]; //get label
		//print_r($attributeField);
		//print_r($this->getScriptsa());
		/* foreach($attributeField as $x){
			echo $x;
			
		}  */
		//$brg=Barang::find()->where(['KD_CORP'=>'ESM','PARENT'=>1])->All(); 
		/* $brgEsmProdakColumn= new ArrayDataProvider([			
			//'key' => 'KD_BARANG',
			'allModels'=>$this->getEsmbrg(),
			 'pagination' => [
				'pageSize' => 20,
			]
		]);
		$clmKdBarang=$brgEsmProdakColumn->allModels[0];  */
