<?php
use kartik\helpers\Html;
use yii\widgets\DetailView;
use kartik\grid\GridView;
use yii\helpers\Url;
use yii\widgets\Pjax;
use yii\bootstrap\Modal;
use yii\bootstrap\ActiveForm;
use kartik\tabs\TabsX;
use yii\helpers\Json;
use yii\web\Response;
use yii\helpers\ArrayHelper;
use yii\web\Request;
use kartik\daterange\DateRangePicker;
use yii\db\ActiveRecord;
use yii\data\ArrayDataProvider;
use dosamigos\gallery\Gallery;
	
	//print_r($model);
	$gambarkosong="data:image/jpg;charset=utf-8;base64,";
	
	$getImg='';
	if(!$model){
		$items[] = [
					//'src'=>'data:image/jpg;charset=utf-8;base64,'.$getImg,
					'src'=>$gambarkosong,
					'imageOptions'=>[
						'width'=>"120px",
						'height'=>"120px",
						'padding-bottom'=>'20px',
						'class'=>'img-rounded'
					], 
					'options'=>[
						//'title' => 'IMAGE START-'. $value['CUST_NM']
					], 
			];
	}else{
		foreach ($model as $key => $value) {
		$getImg=$value['IMGBASE64'];
		$items[] = [
					//'src'=>'data:image/jpg;charset=utf-8;base64,'.$getImg,
					'src'=>$getImg,
					'imageOptions'=>[
						'width'=>"120px",
						'height'=>"120px",
						'padding-bottom'=>'20px',
						'class'=>'img-rounded'
					], 
					'options'=>[
						'title' => $value['judulRoad']
					], 
			];
	};		
	}
		

	/* 2 amigos two galerry author mix:wawan and ptr.nov ver 1.0*/
	$viewItemImge =dosamigos\gallery\Gallery::widget([
		  'items' =>  $items]);
	echo Html::panel([
			'heading' =>false,// '<div> LIST IMAGE </div>',
			'body'=>$viewItemImge . '<br><div style="font-family:tahoma,arial, sans-serif;font-size:9pt; color:red;"><b style="font-size:12pt;">*</b><b style="font-size:8pt;"> Klik Gambar, untuk memulai Slide Show.</b></div>'
		],
		Html::TYPE_INFO
	);
?>