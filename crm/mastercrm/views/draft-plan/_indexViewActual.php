...
$JSEventClick = <<<EOF
	function(calEvent, jsEvent, view) {
		...
		var group = calEvent.scdl;
		 $.get("/mastercrm/draft-plan/get-data-actual?id="+id+"&userid="+user+"&grp="+grp, function( data ) {
            	 var peopleHTML = "";
           ...
           $("#set-out-case-id").attr("href","/mastercrm/draft-plan/set-out-case?tgl="+id+"&username="+name+"&group="+group+"&grpid="+split+"&userid="+user);
...
EOF;
	...
	*/
	$calenderActual=yii2fullcalendar\yii2fullcalendar::widget([
		...
		// 'events'=> $events,
		'ajaxEvents' => Url::to(['/mastercrm/draft-plan/jsoncalendar-actual']),
		'clientOptions' => [
			'selectable' => true,
			'selectHelper' => true,
			'droppable' => true,
	...
	$btn_exportActual = Html::a('<i class="fa fa-file-excel-o"></i> Export Excel',
									'/mastercrm/draft-plan/export-modal?flag=0',
									[		
										'data-toggle'=>"modal",		
	...
	$btn_set_outcase = Html::a('<i class="fa fa-magic"></i> SET OUT CASE',
									'/mastercrm/draft-plan/set-out-valid',
									[
										'id'=>'set-out-case-id',		
										'data-toggle'=>"modal",		
